...
Important limitation: OUI matching is an intrusion *signal*, not authentication — MAC addresses are trivially spoofable and modern devices randomize them. See [`SECURITY.md`](SECURITY.md) for the threat model. **Not yet implemented in firmware** — this layer is currently validated in simulation only; `WiFi.scanNetworks()`-based OUI matching on real hardware is planned (see Roadmap).
...
### 3.4 Fault handling and fail-safe behaviour (implemented and merged into `firmware/sahapilot_firmware.ino`)

Sensor and control faults are treated as first-class states rather than being folded into a "looks fine" value:

- **Ultrasonic sensor fault vs. open-field timeout:** a stuck-HIGH echo line (a genuine hardware fault) and a plain no-object-in-range timeout (open space — not a fault) are now distinguished. 3 consecutive stuck-HIGH reads stop the vehicle immediately and print a `[KRİTİK]` log; a plain timeout is treated as "probably open space" and only stops the vehicle after 15 consecutive occurrences, acting as a fail-safe net rather than a false alarm in wide-open areas.
- **Line lost:** a lost line is reported separately from "perfectly centered" (both used to be error = 0). Short dropouts hold the last correction; after 10 consecutive frames the vehicle stops instead of drifting on stale PD state.
...
This fault-handling logic was verified during development with a C++ test harness that compiles and runs the actual `.ino` file against a mock Arduino API (11 assertions covering baseline driving, stuck-echo shutdown, the open-field timeout safety net, out-of-range rejection, obstacle stop, and line-loss shutdown), and separately compiled with zero warnings on real ESP32 hardware (`esp32:esp32@3.3.11`, program memory usage ~21%).
...
├── tests/          Pytest suite for simulasyon/ (99% coverage, mutation-tested)
├── firmware/       ESP32 (Arduino/C++) firmware — fault-tolerant navigation logic implemented and merged;
│                   pin calibration and RF layer still pending (see Roadmap)
...
- [x] Automated pytest suite for simulation modules (99% coverage, mutation-tested)
- [x] Fault-tolerant navigation firmware (sensor fault detection, fail-safe stop) implemented and merged
- [x] Competition application document (OGTY 2026)
- [ ] Physical prototype build (chassis, wiring, sensor calibration)
- [ ] Pin numbers and PD gains calibrated against real hardware/motor response
- [ ] RF security layer implemented in firmware (`WiFi.scanNetworks()` + OUI matching)
