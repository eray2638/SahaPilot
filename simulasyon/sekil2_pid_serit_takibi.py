...
def uret_ve_ciz(dosya_adi="sekil2_pid_serit_takibi.png"):
    ...
    plt.plot(x, r, "k--", linewidth=1.2, label="Referans serit")
    plt.plot(x, y_iyi, linewidth=2.0, label="PD kontrol (Kp=4, Kd=2)")
    plt.plot(x, y_zayif, linewidth=1.4, label="Zayif kontrol (Kp=0.5, Kd=0.05)")
    plt.xlabel("Ilerleme mesafesi (m)")
    plt.ylabel("Yanal konum (m)")
    plt.title("Sekil 2 - PID ile Serit Takibi")
